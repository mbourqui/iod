...
\begin{enumerate}[(a)]
\item\label{pfp}\textbf{Implémenter la méthode de plus forte pente (Algorithme 11.3) à l'aide du logiciel MATLAB. Déterminer la taille du pas en appliquant la recherche linéaire, Algorithme 11.2 (les deux conditions de Wolfe).}
...
\item\label{pas}\textbf{Implémenter une fonction qui donne la taille du pas suivant:}
...
\textbf{Quelle est la nature de ce pas? D'où cette formule vient-elle?}
...
\item\label{questionC}\textbf{Comparer le comportement de l'algorithme en utilisant les pas (\ref{pfp})\ et (\ref{pas}).}
...
\item\label{questionD}\textbf{Comparer la methode de plus forte pente et la methode quasi-Newton (qui est déjà implementée -- Série 3).}
